...
class CrossrefClient:
    """A client for interacting with the Crossref API following best practices."""
    ...
    def __init__(self, email: str, output_file: str, cache_expire_after: int = 86400):
        ...
        self.headers = {
            'User-Agent': f'PythonCrossrefClient/1.0 (mailto:{email})'
        }
    ...
    def _save_result(self, result: Dict):
        ...
        write_header = not os.path.exists(self.output_file)  # Check if the file exists

        with open(self.output_file, 'a', newline='\n', encoding='utf-8') as f:  # Ensure newline='' for proper formatting
    ...
    def _make_request(self, url: str, params: Dict) -> Dict:
        """Make a request to the Crossref API with exponential backoff."""
    ...
    def parse_authors(self, authors: List[Dict]) -> str:
        """Parse author names from the Crossref response."""
        return ', '.join([f"{author.get('given', '')} {author.get('family', '')}" 
                ...
                try:
                    ...
                    if response_data and 'message' in response_data:
                        ...
                        if items:
                            ...
                            ambiguous = (len(items) > 1 and 
                                       items[0].get('score', 0) == items[1].get('score', 0))
                            
                            result = {
                                'item_id': item_id,
                                'matched_title': best_match.get('title', [None])[0],
                                'doi': best_match.get('DOI'),
                                'issn': best_match.get('ISSN', [None])[0] if 'ISSN' in best_match else None,
...
def main():
    parser = argparse.ArgumentParser(description='Search for publications using Crossref API')
    parser.add_argument('output_file', help='Output CSV file path', default='crossref_results.csv')
    parser.add_argument('input_df', help='Input Dataframe path', default='../data/iris_no_id/iris_no_id.parquet')
    parser.add_argument('--cache-expire', type=int, default=86400,
